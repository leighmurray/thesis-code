...
void loop() {
  ...
  if (isServer()){
    if (inputBufferLeft.available() >= 1 && inputBufferRight.available()){
      ...
      memcpy(&audioPacket[0], inputBufferLeft.readBuffer(), 256);
      memcpy(&audioPacket[256], inputBufferRight.readBuffer(), 256);
      udp.send(remoteIP,  kAudioPort, audioPacket, 512);
  ...
  if (!isServer()){
    ...
    if (0 < size && size <= sizeof(buf)) {
      udp.read(buf, size);
      memcpy(outputBufferLeft.getBuffer(), &buf[0], 256);
      memcpy(outputBufferRight.getBuffer(), &buf[256], 256);
